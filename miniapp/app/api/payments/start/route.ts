...
export async function POST(req: Request) {
  ...
  try {
    ...
    // Prepare insert payload (exact keys matching database schema)
    // Convert amount to numeric for DB (Supabase expects numeric, not string)
    ...
    const dbInsertPayload: Record<string, any> = {
      ...
      method: method,
      plan_code: planCode,
      amount: amountNumeric, // Use numeric value
      currency: "RUB",
      status: "created",
    ...
    // Include out_sum for backward compatibility if column exists
    // This prevents 23502 errors if legacy out_sum column is still present
    // We'll populate it from amount to ensure compatibility
    dbInsertPayload.out_sum = amountNumeric;

    console.log(`[payments/start:${requestId}] Insert payload keys:`, Object.keys(dbInsertPayload));
    ...
    if (insertError || !paymentRecord) {
      ...
      console.error(`[payments/start:${requestId}] Insert payload was:`, dbInsertPayload);
      console.error(`[payments/start:${requestId}] DB error details:`, {
      ...
      return NextResponse.json(
        {
          ...
          dbInsertPayload: dbInsertPayload,
          requestId,
          debug: {
    ...
    return NextResponse.json({
      ...
      debug: {
        ...
        configConfigured: configStatus.configured,
        envVarStatus: configStatus.envVarStatus,
        dbInsertPayloadKeys: Object.keys(dbInsertPayload),
        dbInsertPayload: {
          // Include sanitized payload for debugging (no secrets)
          amount: dbInsertPayload.amount,
          out_sum: dbInsertPayload.out_sum,
          currency: dbInsertPayload.currency,
          method: dbInsertPayload.method,
          plan_code: dbInsertPayload.plan_code,
          status: dbInsertPayload.status,
          provider: dbInsertPayload.provider,
          payment_url: dbInsertPayload.payment_url?.substring(0, 100) + "...", // Truncated
        },
